    ...
    try {
      ...
      if (!entry) {
        console.log(
          ...
          parsed.fileIndex,
          "\n  Registered entries:",
          streamManager.list().map((e,) =>
            `${e.infoHash}:${e.fileIndex} (${e.file.name})`
          ),
        );
      ...
      const ensureStream = () => {
        if (!fileStream) {
          console.log(
            "[main] ensureStream: creating ReadableStream, start:",
            rangeStart,
            "end:",
            rangeEnd,
          );
          ...
          console.log(
            "[main] ensureStream: stream created, file.length:",
            file.length,
          );
        }
      };
      ...
      chunkPort.onmessage = async (ev: MessageEvent,) => {
        console.log(
          "[main] chunkPort.onmessage FIRED, data:",
          ev.data,
          "closed:",
          closed,
        );
        ...
        if (data === true) {
          console.log(
            "[main] SW wants chunk, currentOffset:",
            currentOffset,
            "rangeEnd:",
            rangeEnd,
          );
          ...
          console.log(
            "[main] fileIterator.read() returned, done:",
            done,
            "value:",
          ...
          if (done || !value || value.byteLength === 0) {
            console.log(
              "[main] stream done, sending null, read bytes:",
      ...
      console.log(
        "[main] Response metadata sent, body=STREAM, range:",
        rangeStart,
        "-",
        rangeEnd,
      );
    ...
    try {
      ...
      if (activeWorker) {
        if (activeWorker.state === "activated") {
          ...
          await establishSWConnection();
          console.log(
            "[main] SW MessageChannel established (after activation)",
          );
        }
      } else {
