...
async function prepareAndBuild() {
  ...
  if (!result_worker.success) {
    throw new Error("Falha ao gerar bundle do Worker.",);
  }
  ...
  if (!result_sw.success) {
    throw new Error("Falha ao gerar bundle do Service Worker.",);
  }

  const endTime = performance.now();
  console.log(
    `✅ [DEV SERVER] Build concluído com sucesso em ${
