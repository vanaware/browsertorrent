...
function buildMockDir(
  ...
  const iterable = {
    ...
    next() {
      ...
      const i = (this as unknown as { _idx?: number })._idx =
        (this as unknown as { _idx?: number })._idx ?? 0;
      ...
      if (e.kind === "file") {
        return {
          ...
          value: e.handle ?? mockFileHandle(e.name, new Uint8Array(0,),),
        };
      }
  ...
  return {
    ...
    getFileHandle(name: string,) {
      ...
      const subEntry = entries.find((e,) =>
        e.kind === "directory" && e.name === name
      );
      if (!subEntry || subEntry.kind !== "directory") {
...
Deno.test("calcPieceSize: SIZE_AUTO selects smallest preset > fileSize", () => {
  // 10 KB → SIZE_16KB
  assertEquals(
    calcPieceSize(10 * 1024, PieceSizeEnum.SIZE_AUTO,),
    PieceSizeEnum.SIZE_16KB,
  );
  // 20 KB → SIZE_32KB
  assertEquals(
    calcPieceSize(20 * 1024, PieceSizeEnum.SIZE_AUTO,),
    PieceSizeEnum.SIZE_32KB,
  );
  // 100 KB → SIZE_128KB
  assertEquals(
    calcPieceSize(100 * 1024, PieceSizeEnum.SIZE_AUTO,),
    PieceSizeEnum.SIZE_128KB,
  );
});

Deno.test("calcPieceSize: explicit preset is returned unchanged", () => {
  assertEquals(
    calcPieceSize(1_000_000, PieceSizeEnum.SIZE_512KB,),
    PieceSizeEnum.SIZE_512KB,
  );
  assertEquals(
    calcPieceSize(99, PieceSizeEnum.SIZE_8MB,),
    PieceSizeEnum.SIZE_8MB,
  );
});
  ...
  const root = buildMockDir([
    {
      name: ".DS_Store",
      kind: "file",
      handle: mockFileHandle(".DS_Store", new Uint8Array(0,),),
    },
    {
      name: "visible.txt",
      kind: "file",
      handle: mockFileHandle("visible.txt", new Uint8Array(10,),),
    },
  ],);
  ...
  const root = buildMockDir([
    {
      name: "z.txt",
      kind: "file",
      handle: mockFileHandle("z.txt", new Uint8Array(1,),),
    },
    {
      name: "b.txt",
      kind: "file",
      handle: mockFileHandle("b.txt", new Uint8Array(1,),),
    },
    {
      name: "deep",
      kind: "directory",
      entries: [
        {
          name: "a.txt",
          kind: "file",
          handle: mockFileHandle("a.txt", new Uint8Array(1,),),
        },
      ],
    },
  ],);
...
Deno.test("generateTorrent: created-by and creation-date are present", async () => {
  ...
  const decoded = decode(chunks[0]!, { useMap: true, },) as Map<
    string,
    unknown
  >;
