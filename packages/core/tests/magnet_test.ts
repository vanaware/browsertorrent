...
Deno.test("magnet: parse with multiple trackers", () => {
  const uri =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&tr=udp%3A%2F%2Ftracker.example.com%3A6969&tr=wss%3A%2F%2Ftracker.btorrent.xyz";
  const parsed = parseMagnet(uri,);
...
Deno.test("magnet: parse web seeds and peers", () => {
  const uri =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&ws=https%3A%2F%2Fwebtorrent.io%2Ftorrents%2F&x.pe=192.168.1.1%3A6881";
  const parsed = parseMagnet(uri,);
...
Deno.test("magnet: parse torrent file URL (xs)", () => {
  const uri =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&xs=https%3A%2F%2Fwebtorrent.io%2Ftorrents%2Fsintel.torrent";
  const parsed = parseMagnet(uri,);
  assertEquals(
    parsed.torrentFileUrl,
...
Deno.test("magnet: throws on invalid infoHash length", () => {
  assertThrows(
    () => parseMagnet("magnet:?xt=urn:btih:123",),
    Error,
    "no valid xt hash",
  );
});
...
Deno.test("magnet: isValidMagnet returns true for valid URIs", () => {
  ...
  assertEquals(
    isValidMagnet("magnet:?xt=urn:btmh:1220" + "a".repeat(64,),),
    true,
  );
});
...
Deno.test("magnet: isSha1Base32", () => {
  ...
  assertEquals(
    isSha1Base32("08ada5a7a6183aae1e09d831df6748d566095a10",),
    false,
  ); // hex
...
Deno.test("magnet: parse option validation", () => {
  ...
  assertThrows(
    () =>
      parseMagnet("magnet:?xt=urn:btih:" + "a".repeat(40,), {
...
Deno.test("magnet: params map contains all query parameters", () => {
  const uri =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&dn=Test&tr=udp%3A%2F%2Ft1&tr=udp%3A%2F%2Ft2";
  const parsed = parseMagnet(uri,);
  assertEquals(parsed.params.get("xt",), [
    "urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10",
  ],);
...
Deno.test("magnet: safe decode on malformed percent-encoding", () => {
  const uri =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&dn=bad%XXvalue";
  const parsed = parseMagnet(uri,);
