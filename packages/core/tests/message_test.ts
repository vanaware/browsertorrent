...
Deno.test("message: port rejects invalid port", () => {
  assertThrows(
    () => encodeMessage({ type: "port", port: 70000, },),
    RangeError,
  );
...
Deno.test("message: hashes round-trip", () => {
  ...
  const h = msg as unknown as HashRequestFields & {
    type: "hashes";
    hashes: Uint8Array;
  };
...
Deno.test("message: hashReject round-trip", () => {
  ...
  assertEquals(
    (msg as unknown as HashRequestFields & { type: "hashReject" }).baseLayer,
    3,
  );
});
...
Deno.test("message: have encodes with correct structure", () => {
  ...
  const pieceIndex = new DataView(frame.buffer, frame.byteOffset,).getUint32(
    5,
  );
