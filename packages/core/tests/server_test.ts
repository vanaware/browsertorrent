...
class MockChunkStore {
  chunkLength = 16384;
  get(_index: number,) {
    return Promise.resolve(new Uint8Array(16384,),);
  }
  put(_index: number, _buf: Uint8Array,) {
...
Deno.test("InProcessTransport: records postMessage", () => {
  ...
  assertEquals(
    (transport.sent[0] as unknown as { type: string }).type,
    "WEBTORRENT_ACK",
  );
...
Deno.test("InProcessTransport: sendPull sends signal on port", () => {
  ...
  const fakePort = {
    addEventListener(_: string, cb: (e: { data: boolean },) => void,) {
      ...
      (fakePort as unknown as { _cb: (e: { data: boolean },) => void })._cb =
        cb;
    },
    start() {},
    close() {},
    postMessage(data: boolean,) {
      received = data;
    },
  } as unknown as MessagePort;
