...
import { parseMetainfo, } from "../src/utils/metainfo-parser.ts";
...
Deno.test("metainfo-parser: parses multi-file v1 torrent", async () => {
  ...
  const files =
    (torrent.info as { files: Array<{ length: number; path: string[] }> })
      .files;
...
Deno.test("metainfo-parser: parses single-file v2 torrent", async () => {
  ...
  assertEquals(
    (torrent.info as { "meta version"?: number })["meta version"],
    2,
  );
