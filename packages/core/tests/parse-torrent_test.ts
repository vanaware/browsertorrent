...
import { ParsedTorrent, parseTorrent, } from "../src/utils/parse-torrent.ts";
import { BencodeValue, encode, } from "../src/utils/bencode.ts";
    ...
    info["files"] = [
      {
        length: 1000,
        path: [
          TEXT_ENCODER.encode("folder",),
          TEXT_ENCODER.encode("file1.txt",),
        ],
      },
      {
        length: 2000,
...
Deno.test("parse-torrent: parse magnet URI", async () => {
  const magnet =
    "magnet:?xt=urn:btih:08ada5a7a6183aae1e09d831df6748d566095a10&dn=Sintel&tr=udp%3A%2F%2Ftracker.example.com";
