...
Deno.test("torrent: announce returns the tracker list", async () => {
  const parsed: ParsedTorrent = {
    ...
    announce: [
      ...
      "http://tracker2.example:8080/announce",
    ],
  };
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
...
Deno.test("torrent: rescanFiles() does not double-count pieces", async () => {
  ...
  const piece1 = new Uint8Array(parsed.pieceLength,).fill(1,);
  await torrent.receivePiece(0, piece1,);
  assertEquals(torrent.pieces[0]?.downloaded, true,);
  assertEquals(torrent.progress, 0.5,);
...
// ============================================================================
// PHASE 6: timeRemaining / maxWebConns
// ============================================================================

Deno.test("torrent: timeRemaining returns null when downloadSpeed is 0", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
  ...
  // downloadSpeed starts at 0, so timeRemaining should be null
  assertEquals(torrent.timeRemaining, null,);
});

Deno.test("torrent: timeRemaining returns null when progress is 1 (done)", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
  ...
  // Even if downloadSpeed were > 0, progress >= 1 means timeRemaining is null
  assertEquals(torrent.progress, 1,);
  assertEquals(torrent.timeRemaining, null,);
});

Deno.test("torrent: timeRemaining returns a number when downloadSpeed > 0 and progress < 1", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
...
Deno.test("torrent: timeRemaining calculates correctly with partial progress", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
...
Deno.test("torrent: maxWebConns returns default 10 when no swarm", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
  ...
  assertEquals(torrent.maxWebConns, 10,);
});

Deno.test("torrent: maxWebConns returns swarm maxConns when set", async () => {
  const parsed = await createFakeParsedTorrent();
  const store = new MemoryChunkStore({
    chunkLength: parsed.pieceLength,
    length: parsed.length,
  ...
  assertEquals(torrent.maxWebConns, 5,);
});
