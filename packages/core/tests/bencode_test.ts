...
Deno.test("bencode: decode rejects invalid integer format", () => {
  assertThrows(
    () => decode(te.encode("i01e",),),
    BencodeDecodeError,
    "invalid integer",
  );
  assertThrows(
    () => decode(te.encode("i-0e",),),
    BencodeDecodeError,
    "invalid integer",
  );
  assertThrows(
    () => decode(te.encode("i1.5e",),),
    BencodeDecodeError,
    "invalid integer",
  );
});
...
Deno.test("bencode: decode rejects duplicate dictionary keys", () => {
  assertThrows(
    () => decode(te.encode("d3:fooi1e3:fooi2ee",),),
    BencodeDecodeError,
    "duplicate",
  );
});
...
Deno.test("bencode: decode rejects unsorted dictionary keys", () => {
  assertThrows(
    () => decode(te.encode("d3:zooi1e1:ai1ee",),),
    BencodeDecodeError,
    "not sorted",
  );
});
...
Deno.test("bencode: decode maxBytes rejects oversized input", () => {
  assertThrows(
    () => decode(te.encode("i1e",), { maxBytes: 1, },),
    BencodeDecodeError,
...
Deno.test("bencode: decode useMap returns Map", () => {
  const result = decode(te.encode("d3:foo3:bare",), {
    useMap: true,
  },) as BencodeMap;
...
Deno.test("bencode: encode rejects unsafe integer", () => {
  assertThrows(
    () => encode(1.5 as unknown as BencodeValue,),
    BencodeEncodeError,
    "safe integer",
  );
});
...
Deno.test("bencode: encode rejects unsupported types", () => {
  assertThrows(
    () => encode(null as unknown as BencodeValue,),
    BencodeEncodeError,
    "unsupported",
  );
});
