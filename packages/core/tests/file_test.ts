...
Deno.test("file: createReadStream rejects invalid range", () => {
  ...
  assertThrows(
    () => file.createReadStream({ start: 300, end: 100, },),
    RangeError,
  );
});
...
Deno.test("file: Symbol.asyncIterator emits 'iterator' event", async () => {
  ...
  let eventFired = false;
  file.addEventListener("iterator", () => {
    eventFired = true;
  },);
...
Deno.test("file: emits download event when torrent receives matching piece", () => {
  ...
  const mockTorrent = {
    ...
    pieces: { _data: Uint8Array; get(i: number,): boolean };
    emit: () => void;
  };
  ...
  file.emit(
    "download",
    new CustomEvent("download", { detail: { bytes: 512, }, },),
  );
...
Deno.test("file: emits upload event when torrent forwards upload for matching piece", () => {
  ...
  file.emit(
    "upload",
    new CustomEvent("upload", { detail: { bytes: 256, }, },),
  );
