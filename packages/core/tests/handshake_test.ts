...
Deno.test("handshake: hasExtension returns false for non-8-byte reserved", () => {
  assertEquals(
    hasExtension(new Uint8Array(4,), HandshakeExtension.Dht,),
    false,
  );
});
...
Deno.test("handshake: setExtension and hasExtension", () => {
  ...
  assertEquals(
    hasExtension(reserved, HandshakeExtension.ExtensionProtocol,),
    false,
  );
