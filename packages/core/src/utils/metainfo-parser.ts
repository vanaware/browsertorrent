...
function validateAnnounceList(value: unknown,): boolean {
  return (
    ...
    value.every((tier,) =>
      ...
      tier.every((tracker,) =>
        typeof tracker === "string" && tracker.length > 0
      )
    )
  );
}
...
export async function parseMetainfo(
  ...
  if (
    !isSafeInteger(infoDict["piece length"],) || infoDict["piece length"] <= 0
  ) {
    throw new TorrentParseError(
  ...
  if (
    ...
    throw new TorrentParseError(
      'Invalid "info.private" field — expected 0 or 1',
    );
  }
  ...
  if (infoDict["meta version"] === 2) {
    ...
    if (pieceLayers !== undefined) {
      throw new TorrentParseError(
        'BEP-3 torrent must not contain "piece layers"',
      );
    }
