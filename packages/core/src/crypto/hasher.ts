...
class Sha1Hasher implements IncrementalHasher {
  ...
  digest(): Uint8Array {
    ...
    if (blockLength === 128) {
      this.#processBlock(finalBlock.subarray(64,), state,);
    }

    const result = new Uint8Array(20,);
    const view = new DataView(result.buffer,);
    for (let i = 0; i < state.length; i++) {
      view.setUint32(i * 4, state[i]!, false,);
    }
    return result;
  }
  ...
  #processBlock(block: Uint8Array, state = this.#state,): void {
    ...
    for (let i = 16; i < 80; i++) {
      const value = words[i - 3]! ^ words[i - 8]! ^ words[i - 14]! ^
        words[i - 16]!;
...
export function toHex(digest: Uint8Array,): string {
  return Array.from(digest,).map((b,) => b.toString(16,).padStart(2, "0",))
    .join("",);
}
