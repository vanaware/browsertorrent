...
export class UtPexExtension extends Extension {
    ...
    if (handshakeMap && typeof handshakeMap[UT_PEX_NAME] === "number") {
      ...
      this.emit(
        ...
        new CustomEvent("info", {
          detail: {
            message:
              `ut_pex registered with extension ID: ${this._extensionId}`,
          },
        },),
      );
    } else {
  ...
  public send(update: PexUpdate,): Promise<void> {
    if (this._extensionId === null) {
      throw new Error(
        "ut_pex is not registered (no extension ID from extended handshake)",
      );
    }
    ...
    if (now - this._lastSentAt < this.minSendIntervalMs) {
      throw new Error(
        `ut_pex updates may not be sent this frequently (wait ${
          this.minSendIntervalMs - (now - this._lastSentAt)
        }ms)`,
      );
    }
...
export function encodePexUpdate(update: PexUpdate,): Uint8Array {
  ...
  if (added4.length) {
    ...
    dictionary["added.f"] = Uint8Array.from(
      added4,
      (peer,) => peer.flags ?? 0,
    );
  }
  if (added6.length) {
    ...
    dictionary["added6.f"] = Uint8Array.from(
      added6,
      (peer,) => peer.flags ?? 0,
    );
  }
