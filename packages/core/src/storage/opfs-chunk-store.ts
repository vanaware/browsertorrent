...
export class OPFSChunkStore implements ChunkStore {
  ...
  constructor(opts: ChunkStoreOptions,) {
    ...
    if (!this.rootDir) {
      console.warn(
        "[OPFSChunkStore] OPFS not available, falling back to memory store",
      );
  ...
  get(
    ...
    optsOrCb?:
      ...
      | ((err: Error | null, buf?: Uint8Array,) => void),
    cb?: (err: Error | null, buf?: Uint8Array,) => void,
  ...
  private async _getAsync(
    ...
    try {
      ...
      if (buf.length !== expectedLength) {
        throw new Error(
          `Chunk ${index} has invalid length: expected ${expectedLength}, got ${buf.length}`,
        );
      }
  ...
  private async _putAsync(index: number, buf: Uint8Array,): Promise<void> {
    ...
    if (buf.length !== expectedLength) {
      throw new Error(
        `Invalid chunk length: expected ${expectedLength}, got ${buf.length}`,
      );
    }
  ...
  private async _destroyAsync(): Promise<void> {
    ...
    try {
      for await (
        const entry of (this.rootDir as unknown as {
