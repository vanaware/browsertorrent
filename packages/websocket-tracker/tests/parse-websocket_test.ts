...
describe("parseWebSocketMessage", () => {
  it("deve retornar null para mensagem vazia", () => {
    assertEquals(parseWebSocketMessage("",), null,);
  });

  it("deve retornar null para JSON inválido", () => {
    assertEquals(parseWebSocketMessage("not json",), null,);
  });

  it("deve retornar null para mensagem sem action", () => {
    assertEquals(
      parseWebSocketMessage('{"info_hash":"abc","peer_id":"def"}',),
      null,
    );
  });
  ...
  it("deve retornar null para mensagem sem info_hash", () => {
    assertEquals(
      parseWebSocketMessage('{"action":"announce","peer_id":"def"}',),
      null,
    );
  });
  ...
  it("deve retornar null para mensagem sem peer_id", () => {
    assertEquals(
      parseWebSocketMessage('{"action":"announce","info_hash":"abc"}',),
      null,
    );
  });
  ...
  it("deve retornar null para action desconhecida", () => {
    assertEquals(
      parseWebSocketMessage(
        '{"action":"unknown","info_hash":"abc","peer_id":"def"}',
      ),
      null,
    );
  });
...
describe("formatAnnounceResponse", () => {
  it("deve formatar resposta announce corretamente", () => {
    const response = formatAnnounceResponse("hash123", "peer123", [
      "1.2.3.4:6881",
    ],);
