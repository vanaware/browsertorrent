...
import { type ITracker, TrackerRouter, } from "../src/router.ts";

// Mock tracker que implementa a interface mínima para testar o router
class MockTracker implements ITracker {
  handleHttpAnnounce(url: URL, _req: Request,): Response {
    return new Response(JSON.stringify({ action: "announce", },), {
      headers: { "Content-Type": "application/json", },
    },);
  }

  handleHttpScrape(url: URL, _req: Request,): Response {
    return new Response(JSON.stringify({ action: "scrape", },), {
      headers: { "Content-Type": "application/json", },
    },);
  }
...
describe("TrackerRouter", () => {
  ...
  it("deve rotear para handleHttpAnnounce no path /announce", async () => {
    ...
    const req = new Request(
      "http://localhost:8000/announce?info_hash=abc&peer_id=def&port=6881",
    );
  ...
  it("deve passar URL com query params para announce", async () => {
    ...
    const tracker: ITracker = {
      ...
      handleHttpScrape: () => new Response("ok",),
      handleConnection: () => {},
    };
    const router = new TrackerRouter(tracker,);
    ...
    assertEq(capturedUrl !== null, true,);
    if (capturedUrl) {
      const urlObj = capturedUrl as URL;
      assertEquals(urlObj.searchParams.get("info_hash",), "abc123",);
      assertEquals(urlObj.searchParams.get("peer_id",), "def456",);
      assertEquals(urlObj.searchParams.get("port",), "6881",);
      assertEquals(urlObj.searchParams.get("event",), "started",);
    }
  });
});
