...
describe("WebSocketTracker", () => {
  ...
  it("deve retornar stats após handleHttpAnnounce", async () => {
    ...
    const req = new Request(
      "http://localhost:8000/announce?info_hash=abc&peer_id=def&port=6881&event=started",
    );
    const res = await tracker.handleHttpAnnounce(
      new URL(
        "http://localhost:8000/announce?info_hash=abc&peer_id=def&port=6881&event=started",
      ),
      req,
  ...
  it("deve retornar erro para handleHttpAnnounce sem info_hash", async () => {
    const tracker = new WebSocketTracker();
    const req = new Request(
      "http://localhost:8000/announce?peer_id=def&port=6881",
    );
    const res = await tracker.handleHttpAnnounce(
      new URL("http://localhost:8000/announce?peer_id=def&port=6881",),
      req,
    );
    ...
    assertEquals(
      body.error,
      "Missing required parameters: info_hash and peer_id",
    );
  });
  ...
  it("deve retornar stats após handleHttpScrape", async () => {
    ...
    const res = await tracker.handleHttpScrape(
      new URL("http://localhost:8000/scrape?info_hash=abc",),
      req,
    );
    assertEquals(res.status, 200,);
    const body = await res.json();
  ...
  it("deve retornar erro para handleHttpScrape sem info_hash", async () => {
    ...
    const res = await tracker.handleHttpScrape(
      new URL("http://localhost:8000/scrape",),
      req,
    );
    assertEquals(res.status, 400,);
    const body = await res.json();
