    ...
    ws.addEventListener("message", (e,) => {
      ...
      if (msg.action === "announce") {
        ...
        if (typeof msg.interval !== "number") {
          reject(
            new Error(`Missing interval in announce response from ${peerId}`,),
          );
          return;
        }
        if (typeof msg.complete !== "number") {
          reject(
            new Error(`Missing complete in announce response from ${peerId}`,),
          );
          return;
        }
        if (typeof msg.incomplete !== "number") {
          reject(
            new Error(
              `Missing incomplete in announce response from ${peerId}`,
            ),
          );
          return;
        }
        if (!Array.isArray(msg.peers,)) {
          reject(
            new Error(
              `Missing peers array in announce response from ${peerId}`,
            ),
          );
          return;
        }
        ...
        for (const p of msg.peers) {
          if (!p.peer_id || !p.ip || !p.port) {
            reject(
              new Error(`Invalid peer object format: ${JSON.stringify(p,)}`,),
            );
            return;
          }
        }
